...
double time_diff(struct timeval tstart,struct timeval tend){

    return 1000000*(tend.tv_sec-tstart.tv_sec)+tend.tv_usec-tstart.tv_usec;//ms
...
//update by liwenhao : add RF code***********************************************************************************************
...
//从文件中将数据读取到树中
void read_file(struct Node *tree,FILE * file)
{
    ...
    fscanf(file,"%d\t%d\t%d\t%lf\n",&tree->label,&tree->isleaf,&tree->key,&tree->criterion);
...
int *predict(Node *tree, double **vectors, int n_vectors) {
    ...
    char *model_file=getenv("MODEL_FILE");//get model file name
    if(model_file==NULL){
        printf("We will use the default model: default_model. If you want to use the self-defining model, please export model file name like: export MODEL_FILE = model_path/model\n");
        model_file="default_model";
    }
    printf("Model file:%s\n",model_file);
    // printf("if you want to set the threshold of the gemm, you can do like this: \"export GEMM_THRESHOLD=x\", while the x is the number of the GEMM_THRESHOLD\n");
    file = fopen(model_file,"r");
    if(file == NULL)
	{
        printf("The model file is NULL, can not open!");
        return 0;
	}
...
//update by liwenhao : end RF code************************************************************************************************************
...
/* #pragma omp parallel */ /* Sherry -- parallel done inside pdgstrs2 */
		...
		* for outer-product update.                        *
        ...
        //update by liwenhao
        char *threshold_data=getenv("GEMM_THRESHOLD");
        int GEMM_THRESHOLD_NUM=threshold_data==NULL?0:atoi(threshold_data);
        #include "spgemm.c"
...
//#include "dSchCompUdt-2Ddynamic_v6.c"
  ...
  #include "dSchCompUdt-2Ddynamic.c"
    ...
    //update by liwenhao
        stat->utime[FACT] = pxgstrfTimer;
        LookAheadGEMMTimer+=look_ahead_gemm;
        NetSchurUpTimer+=look_ahead_gemm;
